...
(function(Firebase) {
  ...
  function Firechat(firebaseRef, options) {
    ...
    // NkChat: moderators, organization online references added
    this._moderatorsOnlineRef = this._firebase.child('moderators-online');
    this._organizationsAdminsOnlineRef = this._firebase.child('organization-admins-online');
  ...
  Firechat.prototype = {
    ...
    _loadUserMetadata: function(onComplete) {
      ...
      this._userRef.transaction(function(current) {
        ...
        // NkChat: mod status from auth role
        self._isModerator = self._role == ROLE_MODERATOR;
        // NkChat: not used:
    ...
    _setupDataEvents: function() {
      ...
      // NkChat: register moderators in the public moderator listing.
      if(this._isModerator) {
          var moderatorRef = this._moderatorsOnlineRef.child(this._userName.toLowerCase());
          var moderatorSessionRef = moderatorRef.child(this._sessionId);
          this._queuePresenceOperation(moderatorSessionRef, {
              id: this._userId,
              name: this._userName
          }, null);
      }
      _.each(this._organizations, function (organizationId) {
        var orgAdminSessionRef = self._organizationsAdminsOnlineRef
                                     .child(organizationId)
                                     .child(self._userName.toLowerCase())
                                     .child(self._sessionId);
        self._queuePresenceOperation(orgAdminSessionRef, {
              id: self._userId,
              name: self._userName
          }, null);
      });
  ...
  // NkChat: role, organizations arguments added
  Firechat.prototype.setUser = function(userId, userName, role, organizations, callback) {
    var self = this;
    ...
    self._firebase.root().child('.info/authenticated').on('value', function(snapshot) {
      ...
      if (authenticated) {
        ...
        self._role = role; // NkChat role
        self._organizations = organizations; // NkChat organization ids
  ...
  // NkChat: roomName, roomType -> opts.name, opts.type
  Firechat.prototype.createRoom = function(opts, callback) {
    var self = this,
    ...
    var newRoom = {
      ...
      // NkChat: createdByUserId -> creator object
      creator: {uid: this._userId, name: this._userName},
      createdAt: Firebase.ServerValue.TIMESTAMP
    };

    if (opts.type === 'private') {
      newRoom.target = opts.target;
  ...
  Firechat.prototype.enterRoom = function(roomId) {
    ...
    self.getRoom(roomId, function(room) {
      // NkChat: leave deleted (null) room
      if(room === null) {
      ...
      // NkChat: pass full room meta data to callbacks:
      self._onEnterRoom(room);
  ...
  Firechat.prototype.suspendUser = function(userId, timeLengthSeconds, cb) {
    ...
    self._suspensionsRef.child(userId).set(suspendedUntil, function(error) {
      if (error && cb) {
        ...
        // NkChat: invoke callback only if it's provided
        //return cb(null);
        if(cb) {
            return cb(null);
        }
