(function($) {
   ...
   */
  FirechatUI.prototype.setUser = function(userId, userName, role, organizations) {
    var self = this;

    // Initialize data events
    // NkChat: role, organizations arguments added
    self._chat.setUser(userId, userName, role, organizations, function(user) {
      self._user = user;
        ...
        renderPrivateInvitePrompt = function(event) {
          ...
          if (userId && userName) {
            ...
            $prompt.find('[data-toggle=accept]').first().click(function() {
              ...
              self._chat.createRoom({name: roomName, type: 'private'}, function(roomId) {
  ...
  FirechatUI.prototype._bindForRoomListing = function() {
    ...
    $createRoomButton.bind('click', function(event) {
      ...
      // NkChat: roomName -> opts.name
      self._chat.createRoom({name: roomName});
      return false;
    });
  };
  ...
  FirechatUI.prototype.promptCreateRoom = function() {
    ...
    $prompt.find('[data-toggle=submit]').first().click(function() {
      ...
      if (name !== '') {
        // NkChat: roomName -> opts.name, type -> opts.type
        self._chat.createRoom({name: name, type: 'public'});
        $prompt.remove();
      }
      return false;
    });
